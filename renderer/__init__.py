    ...
    class CallbacksWithContextImpl(ftml.Callbacks):
        ...
        def render_user(self, username: str, avatar: bool) -> str:
            try:
                ...
                return render_template_from_string(
                    '<span class="error-inline">用户 \'{{username}}\' 不存在</span>',
                    username=username
                )
        ...
        def get_i18n_message(self, message_id: str) -> str:
            messages = {
                ...
                "collapsible-open": "+ 打开折叠",
                "collapsible-hide": "- 关闭折叠",
                "table-of-contents": "内容",
                "toc-open": "展开",
                "toc-close": "关闭",
                "footnote": "脚注",
                "footnote-block-title": "脚注",
                "image-context-bad": "图像地址不正确",
            }
        ...
        def render_include_not_found(self, full_name: str) -> str:
            ...
            if include_name in threadvars.get('include_err', []):
                return '[[div class="error-block"]]插入的页面 "%s" 导致了无限包含循环[[/div]]' % full_name
            else:
                # this must return Wiki markup because of the stage it runs at.
                return '[[div class="error-block"]]插入的页面 "%s" 不存在 ([[a href="/%s/edit/true" target="_blank"]]立刻创建[[/a]])[[/div]]' % (full_name, full_name)
