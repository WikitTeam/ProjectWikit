...
def log_entry_type_name(entry: ArticleLogEntry.LogEntryType) -> tuple[str, str]:
    mapping = {
        ArticleLogEntry.LogEntryType.Source: ('S', '文章内容已更改'),
        ArticleLogEntry.LogEntryType.Title: ('T', '标题已更改'),
        ArticleLogEntry.LogEntryType.Name: ('R', '页面已重命名/删除'),
        ArticleLogEntry.LogEntryType.Tags: ('A', '标签已更改'),
        ArticleLogEntry.LogEntryType.New: ('N', '新页面已创建'),
        ArticleLogEntry.LogEntryType.Parent: ('M', '父页面已更改'),
        ArticleLogEntry.LogEntryType.FileAdded: ('F', '文件已添加'),
        ArticleLogEntry.LogEntryType.FileDeleted: ('F', '文件已删除'),
        ArticleLogEntry.LogEntryType.FileRenamed: ('F', '文件已重命名'),
        ArticleLogEntry.LogEntryType.VotesDeleted: ('V', '投票已更改'),
        ArticleLogEntry.LogEntryType.Authorship: ('C', '作者已更改'),
        ArticleLogEntry.LogEntryType.Wikidot: ('W', '从Wikidot移植的编辑')
    }
...
def log_entry_default_comment(entry: ArticleLogEntry) -> str:
    ...
    if entry.type == ArticleLogEntry.LogEntryType.Title:
        return f'标题已从 "{entry.meta['prev_title']}" 更改为 "{entry.meta['title']}"'

    if entry.type == ArticleLogEntry.LogEntryType.Name:
        return f'页面已从 "{entry.meta['prev_name']}" 重命名为 "{entry.meta['name']}"'
    ...
    if entry.type == ArticleLogEntry.LogEntryType.Tags:
        ...
        log = [
            added_tags and f'添加标签：{', '.join(added_tags)}。',
            removed_tags and f'移除标签：{', '.join(removed_tags)}。'
        ]
    ...
    if entry.type == ArticleLogEntry.LogEntryType.Parent:
        if entry.meta['prev_parent'] and entry.meta['parent']:
            return f'父页面已从 "{entry.meta['prev_parent']}" 更改为 "{entry.meta['parent']}"'
        if entry.meta['prev_parent']:
            return f'已移除父页面 "{entry.meta['prev_parent']}"'
        if entry.meta['parent']:
            return f'已设置父页面 "{entry.meta['parent']}"'

    if entry.type == ArticleLogEntry.LogEntryType.FileAdded:
        return f'已上传文件："{entry.meta['name']}"'

    if entry.type == ArticleLogEntry.LogEntryType.FileDeleted:
        return f'已删除文件："{entry.meta['name']}"'

    if entry.type == ArticleLogEntry.LogEntryType.FileRenamed:
        return f'文件已从 "{entry.meta['prev_name']}" 重命名为 "{entry.meta['name']}"'
    ...
    if entry.type == ArticleLogEntry.LogEntryType.VotesDeleted:
        ...
        return f'已重置页面评分：{rating_str}（投票数：{entry.meta['votes_count']}，人气：{entry.meta['popularity']}%）'
    ...
    if entry.type == ArticleLogEntry.LogEntryType.Authorship:
        ...
        log = [
            added_authors and f'添加作者：{', '.join(added_authors)}。',
            removed_authors and f'移除作者：{', '.join(removed_authors)}。'
        ]
        return ' '.join(l for l in log if l)

    if entry.type == ArticleLogEntry.LogEntryType.Revert:
        return f'已回退页面至版本 #{entry.meta['rev_number']}'
    ...
    return render_template_from_string(
        ...
        <div class="site-changes-box w-site-changes" data-site-changes-path-params="{{ data_path_params }}" data-site-changes-params="{{ data_params }}">
            ...
            <form onsubmit="return false;" action="" method="get">
                ...
                <tr>
                    ...
                    <td>
                        <select id="rev-category">
                            <option value="*"{% if category_param == '*' %} selected{% endif %}>所有分类</option>
                ...
                <tr>
                    <td>按用户名筛选：<br><span style="font-size: 75%">使用前缀"~"进行部分<br>用户名匹配筛选</span></td>
                    <td>
                ...
                <div class="buttons">
                    <input class="btn btn-default btn-sm" type="button" value="刷新列表">
                </div>
            </form>
