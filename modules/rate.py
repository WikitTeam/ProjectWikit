    ...
    if mode == Settings.RatingMode.UpDown:
        return render_template_from_string(
            ''.join([
                ...
                '<span class="rate-points">评分:&nbsp;<span class="number prw54353">{{rating}}</span></span>',
                '<span class="rateup btn btn-default"><a title="喜欢" href="#">+</a></span>',
                '<span class="ratedown btn btn-default"><a title="不喜欢" href="#">–</a></span>',
                '<span class="cancel btn btn-default"><a title="取消" href="#">x</a></span>',
                '</div>'
            ]),
        ...
        return render_template_from_string(
            ''.join([
                ...
                '<div class="w-stars-rate-rating">评分:&nbsp;<span class="w-stars-rate-number">{{rating}}</span></div>',
                ...
                '</div>',
                '<div class="w-stars-rate-votes"><span class="w-stars-rate-number" title="评分数">{{votes}}</span>/<span class="w-stars-rate-popularity" title="人气值（3.0分及以上评分占比）">{{popularity}}</span>%</div>',
                '</div>'
            ]),
            page_id=context.article.full_name,
...
def format_time(seconds):
    ...
    if t_seconds or not t_minutes:
        s = pluralize_russian(t_seconds, '秒')
    if t_minutes:
        s = pluralize_russian(t_minutes, '分钟') + ' ' + s
    return s
...
def api_rate(context, params):
    ...
    if not context.user.has_perm('roles.rate_articles', context.article):
        raise ModuleError('权限不足')

    if 'value' not in params:
        raise ModuleError('未指定评分值')
    ...
    if value is not None:
        ...
        if obj_settings.rating_mode == Settings.RatingMode.UpDown:
            if value not in [-1, 0, 1]:
                raise ModuleError('无效的评分 %s' % str(value))
    ...
    try:
        ...
        raise ModuleError(f'您在 {format_time(e.time_total)} 内已为其他文章评分。请于 {format_time(e.time_left)} 后重试。您可利用剩余时间仔细阅读当前待评分的文章。')
