...
def render_pagination(base_path, pagination_page, pagination_total_pages):
    if pagination_total_pages > 1:
        ...
        return render_template_from_string(
            """
            <div class="pager">
                <span class="pager-no">第&nbsp;{{page}}&nbsp页; 共&nbsp;{{total_pages}}&nbsp;页</span>
                {% if show_prev_page %}
                    <span class="target"><a href="{%if base_path%}{{base_path}}/p/{{prev_page}}{% else %}#{%endif%}" data-pagination-target="{{prev_page}}">&laquo;&nbsp;上一页</a></span>
                {% endif %}
                {% for p in left_pages %}
                    {% if page == p %}
                ...
                {% endfor %}
                {% if show_next_page %}
                    <span class="target"><a href="{%if base_path%}{{base_path}}/p/{{next_page}}{% else %}#{%endif%}" data-pagination-target="{{next_page}}">下一页&nbsp;&raquo;</a></span>
                {% endif %}
            </div>
            """,
