...
func TestModuleHasBody(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("ModuleHasBody(%q) err = %v，期望 nil", tt.name, err)
			}
			if got != tt.want {
				t.Errorf("ModuleHasBody(%q) = %v，期望 %v", tt.name, got, tt.want)
			}
		})
	}
}
...
func TestRenderModuleLowercasesParamKeys(t *testing.T) {
	...
	if _, err := c.RenderModule("Rate", map[string]string{"ShowVotes": "yes"}, ""); err != nil {
		t.Fatalf("RenderModule() err = %v，期望 nil", err)
	}
	if repo.moduleName != "Rate" {
		t.Errorf("moduleName = %q，期望 %q", repo.moduleName, "Rate")
	}
	if _, ok := repo.moduleParam["showvotes"]; !ok {
		t.Errorf("params = %v，期望含键 %q", repo.moduleParam, "showvotes")
	}
}
...
func TestRenderModuleTurnsModuleErrorIntoErrorBlock(t *testing.T) {
	...
	if got != want {
		t.Errorf("RenderModule() = %q，期望 %q", got, want)
	}
}
...
func TestRenderModulePropagatesOtherErrors(t *testing.T) {
	...
	if _, err := c.RenderModule("rate", nil, ""); !errors.Is(err, boom) {
		t.Errorf("RenderModule() err = %v，期望 %v", err, boom)
	}
}
...
func TestRenderUserNotFound(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("RenderUser() err = %v，期望 nil", err)
	}
	want := `<span class="error-inline">用户 'kaku&lt;shi' 不存在</span>`
	if got != want {
		t.Errorf("RenderUser() = %q，期望 %q", got, want)
	}
}
...
func TestGetI18nMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.id, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("GetI18nMessage(%q) err = %v，期望 nil", tt.id, err)
			}
			if got != tt.want {
				t.Errorf("GetI18nMessage(%q) = %q，期望 %q", tt.id, got, tt.want)
			}
		})
	}
}
...
func TestGetHTMLInjectedCodeSubstitutesIDOnce(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("GetHTMLInjectedCode() err = %v，期望 nil", err)
	}
	if !strings.Contains(got, `id: "abc-12"`) {
		t.Errorf("输出不含 %q", `id: "abc-12"`)
	}
	if strings.Contains(got, "%s") {
		t.Errorf("输出仍含占位符 %q，期望已被替换", "%s")
	}
	if !strings.HasPrefix(got, "\n    <script>") {
		t.Errorf("输出开头 = %q，期望 %q", got[:min(len(got), 16)], "\n    <script>")
	}
}
...
func TestEvaluateExpressionReportsBoolAsInt(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.src, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("EvaluateExpression(%q) err = %v，期望 nil", tt.src, err)
			}
			if got != tt.want {
				t.Errorf("EvaluateExpression(%q) = %+v，期望 %+v", tt.src, got, tt.want)
			}
		})
	}
}
...
func TestNormalizePageName(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("NormalizePageName() err = %v，期望 nil", err)
	}
	if got != "scp-173" {
...
func TestNextIncludeLevelStopsAtZero(t *testing.T) {
	...
	for i := range MaxIncludeLevel {
		...
		if err != nil {
			t.Fatalf("第 %d 次 NextIncludeLevel() err = %v，期望 nil", i+1, err)
		}
		if !ok {
			t.Fatalf("第 %d 次 NextIncludeLevel() = false，期望 true", i+1)
		}
	}
	ok, err := c.NextIncludeLevel()
	if err != nil {
		t.Fatalf("NextIncludeLevel() err = %v，期望 nil", err)
	}
	if ok {
		t.Errorf("第 %d 次 NextIncludeLevel() = true，期望 false", MaxIncludeLevel+1)
	}
}
...
func TestIncludePagesReturnsEmptyContentAfterOverflow(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("IncludePages() err = %v，期望 nil", err)
	}
	if len(got) != 1 {
		t.Fatalf("len(IncludePages()) = %d，期望 1", len(got))
	}
	if got[0].Content != nil {
		t.Errorf("Content = %v，期望 nil", *got[0].Content)
	}
	if repo.includeSeen != nil {
		t.Error("溢出后仍调用了数据源，期望不调用")
	}
}
...
func TestPureCallbacksWorkWithoutRepository(t *testing.T) {
	...
	if _, err := c.ModuleHasBody("css"); err != nil {
		t.Errorf("ModuleHasBody() err = %v，期望 nil", err)
	}
	if _, err := c.GetI18nMessage("toc-open"); err != nil {
		t.Errorf("GetI18nMessage() err = %v，期望 nil", err)
	}
	if _, err := c.GetHTMLInjectedCode("x"); err != nil {
		t.Errorf("GetHTMLInjectedCode() err = %v，期望 nil", err)
	}
	if _, err := c.EvaluateExpression("1"); err != nil {
		t.Errorf("EvaluateExpression() err = %v，期望 nil", err)
	}
	if _, err := c.NormalizePageName("x"); err != nil {
		t.Errorf("NormalizePageName() err = %v，期望 nil", err)
	}
	if _, err := c.NoSuchInclude("x"); err != nil {
		t.Errorf("NoSuchInclude() err = %v，期望 nil", err)
	}
	if _, err := c.NextIncludeLevel(); err != nil {
		t.Errorf("NextIncludeLevel() err = %v，期望 nil", err)
	}
}
